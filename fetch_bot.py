...
class Bot:
    print('run')
    fetch_admin = Skype(tokenFile='token.txt')  # skype account
    ...
    def update_order(self):
        ...
        for dic in self.sheet:
            if dic['key'].find('link order cơm') >= 0:
                msg += dic['answer']
        self.fetch_group.setTyping(active=True)
    ...
    def notify(self):
        ...
        while True:
            ...
            if now.strftime("%A").lower() in days:
                ...
                if datetime.now().strftime("%H:%M:%S") == "10:07:05":
                    for dic in self.sheet:
                        if dic['key'].find('đặt cơm') >= 0:
                            self.fetch_group.setTyping(active=True)
                            self.fetch_group.sendMsg(dic['answer'], rich=True)
                if datetime.now().strftime("%H:%M:%S") == "11:03:01":
                    self.update_order()
                        ...
                        if content != None:
                            ...
                            if content.startswith('<at id="8:live:cuongdaovan262">Fetch_admin</at>'):
                                ...
                                if mess.find('--help') >= 0:
                                    ...
                                    for key in self.all_key()[1:]:
                                        msg += str(self.all_key().index(key)) + ". " + key + "\n"
                                    self.fetch_group.setTyping(active=True)
                                    self.fetch_group.sendMsg(msg, rich=True)
